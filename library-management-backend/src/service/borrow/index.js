...
export const createBorrowBooks = async (
  ...
  try {
    ...
    if (selectedDueDays < 3 || selectedDueDays > 90) {
      return { code: 400, payload: "Due date must be between 3 and 90 days." };
    }
    ...
    for (const bookInfo of booksToBorrow) {
      ...
      if (availableBooks.length === 0) {
        return { code: 404, payload: `No available books found for ${title}` };
      }
      ...
      borrowedBookIds.push(randomBook._id.toString()); // Chuyển ObjectId thành String
    }

    if (borrowedBookIds.length === 0) {
      return { code: 400, payload: "No books were added to borrow." };
    }
    ...
    const borrowRecord = new Borrow({
      userId,
      books: borrowedBookIds, // Đảm bảo bookId được lưu dưới dạng String
    ...
    console.log("Borrow Record Before Save:", borrowRecord); // Kiểm tra log
...
export const updateBorrow = async (borrowId, updatedData) => {
  try {
    ...
    const allowedFields = ["books", "dueDate", "status", "fine", "borrowDate"];
    ...
    if (updatedData.dueDate || updatedData.borrowDate) {
      ...
      dueDate.setDate(dueDate.getDate() + (updatedData.selectedDueDays || 0));
      const returnDate = new Date(dueDate);
      returnDate.setDate(returnDate.getDate() + 2);
...
export const returnBook = async (borrowId) => {
  try {
    const borrow = await Borrow.findById(borrowId).populate("books");
    ...
    if (borrow.fine > 0) {
      return {
        code: 400,
        payload: `This borrow is overdue. Need to pay the fine of ${borrow.fine} before returning the books.`,
      };
    }
    ...
    const updateBookPromises = borrow.books.map(async (book) => {
      book.status = "NEW";
      await book.save();
    });
    ...
    return {
      code: 200,
      payload: "Books returned successfully.",
    };
  } catch (error) {
    return {
      code: 500,
