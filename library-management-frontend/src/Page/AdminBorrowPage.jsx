...
function AdminBorrowPage() {
  ...
  const fetchBorrows = async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data.payload)) {
        setBorrows(response.data.payload);
        setFilteredBorrows(response.data.payload);
        setErrorMessage("");
      } else {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = borrows.filter((borrow) => {
      ...
      return (
        username.includes(lowercasedTerm) || email.includes(lowercasedTerm)
      );
    });
  ...
  const handleReturnBorrow = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setSuccessMessage(`Borrow ${selectedBorrowId} returned successfully!`);
  ...
  const handleViewDetails = async (borrowId) => {
    try {
      ...
      setErrorMessage(
        error.response?.data?.message || "Failed to fetch borrow details."
      );
    }
  };
  ...
  const handleDeleteBorrow = async (borrowId) => {
    if (
      ...
      try {
        ...
        if (response.status === 200) {
          setSuccessMessage(`Borrow ${borrowId} deleted successfully!`);
          fetchBorrows();
          setTimeout(() => setSuccessMessage(""), 3000);
        }
      } catch (error) {
      ...
      <main className="mx-auto max-w-7xl sm:px-6 sm:pt-16 lg:px-8 relative">
        ...
        <div className="mt-6 overflow-x-auto">
          {paginatedBorrows.length === 0 ? (
            <p className="text-lg text-gray-500">No borrow records found.</p>
          ) : (
                ...
                {paginatedBorrows.map((borrow) => (
                  <tr key={borrow._id}>
                    <td
                      ...
                      onClick={() => handleViewDetails(borrow._id)}
                    >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {borrow?.books?.map((book) => book?.title).join(", ") ||
                        "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {borrow?.status || "N/A"}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {borrow?.status === "BORROWED" && (
                        <button
                          ...
                          onClick={() => handleOpenReturnModal(borrow._id)}
                        >
                          Return
                        </button>
                      )}
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
              ...
              <div className="mb-6 bg-gray-50 p-4 rounded-lg shadow-inner">
                <p className="text-gray-600">
                  <strong>ID:</strong> {selectedBorrow?._id || "N/A"}
                </p>
                <p className="text-gray-600">
                ...
                <p className="text-gray-600">
                  <strong>Phone:</strong>{" "}
                  {selectedBorrow?.userId?.phoneNumber || "N/A"}
                </p>
                <p className="text-gray-600">
                ...
                <ul className="list-disc pl-5 space-y-2">
                  {selectedBorrow?.books?.map((book) => (
                    <li key={book?._id} className="text-gray-600">
                      {book?.title || "N/A"} (ID: {book?._id || "N/A"})
                    </li>
                  ))}
                </ul>
                <p className="text-gray-600 mt-2">
                  ...
                  {selectedBorrow?.borrowDate
                    ? new Date(selectedBorrow.borrowDate).toLocaleString()
                    : "N/A"}
                </p>
                <p className="text-gray-600">
                  <strong>Due Date:</strong>{" "}
                  {selectedBorrow?.dueDate
                    ? new Date(selectedBorrow.dueDate).toLocaleString()
                    : "N/A"}
                </p>
                <p className="text-gray-600">
                  <strong>Return Date:</strong>{" "}
                  {selectedBorrow?.returnDate
                    ? new Date(selectedBorrow.returnDate).toLocaleString()
                    : "Not Returned"}
                </p>
                <p className="text-gray-600">
                  <strong>Status:</strong> {selectedBorrow?.status || "N/A"}
                </p>
                <p className="text-gray-600">
                  <strong>Fine:</strong> {selectedBorrow?.fine || 0}
                </p>
              </div>
